...
describe('Executor Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        usol = await hre.ethers.deployContract('MockERC20', ['uSOL', 'uSOL', 18]);
    ...
    it('should successfully execute a valid trade', async () => {
        ...
        const makerInitialWeth = await settlement.deposits(await maker.getAddress(), usol.target);
    ...
    it('should fail to execute trade with revoked maker approval', async () => {
        await usol.connect(maker).approve(settlement.target, 0);
